...
def classify_error(original_eq_text, last_correct_step_text,
    ...
    system = (
        ...
        "so 9 + (-12) = -3 instead of the correct 9 + 12 = 21). "
        "ALSO applies regardless of which side has x: from '12 = 2x + 4' "
        "the student writes '2x = 16' (moved the +4 to the LHS but kept "
        "its sign as +4, computing 12 + 4 = 16 instead of the correct "
        "12 − 4 = 8). The hallmark is the WRONG MAGNITUDE coming from "
        "using the wrong sign during addition, whether x ends up on the "
        "left or the right of '='. "
        "Do NOT confuse with a stray sign on the right magnitude "
...
def generate_worked_example(last_correct_eq_text, original_eq_text,
    ...
    system = (
        ...
        "numbers."
        "\n\nOUTPUT FORMAT (MANDATORY — exact structure, line by line):"
        "\nThe output must have EXACTLY these 5 lines separated by '<br>' "
        "tags. NO preamble, NO extra prose. Each line plays a precise role:"
        "\n  Line 1: The example equation in LaTeX, e.g. '$3y = 15$'"
        "\n  Line 2: ONE short sentence in Catalan describing the operation "
        "to apply (e.g. 'Per aïllar la incògnita, has de dividir els dos "
        "costats per 3.')"
        "\n  Line 3: The operation applied in LaTeX fraction form, e.g. "
        "'$\\frac{3y}{3} = \\frac{15}{3}$'"
        "\n  Line 4: The result boxed, e.g. '$\\boxed{y = 5}$'"
        "\n  Line 5: The literal sentence 'Ara prova el mateix, amb la "
        "teva equació.'"
        "\nJoin the 5 lines with '<br>' (literally, the HTML tag as text — "
        "the renderer will convert them to line breaks). Do NOT use real "
        "newlines, only '<br>'. The result is ONE single line of text "
        "containing 4 '<br>' separators."
        "\n\nExample of the EXACT expected output for the case 3y=15 → y=5:"
        "\n$3y = 15$<br>Per aïllar la incògnita, has de dividir els dos "
        "costats per 3.<br>$\\frac{3y}{3} = \\frac{15}{3}$<br>"
        "$\\boxed{y = 5}$<br>Ara prova el mateix, amb la teva equació."
        ...
        "\n\nOutput ONLY the worked example text in the format above, no "
        "preamble, no JSON, no markdown code fences."
    )
